...
\begin{parts}
    ...
    \answer{
        In file \href{run:../utils.py}{utils.py}
    }
    ...
    \answer{
        In file \href{run:../model\_embeddings.py}{model\_embeddings.py}
    }
    ...
    \answer{
        In file \href{run:../nmt\_model.py}{nmt\_model.py}
    }
...
\end{lstlisting}
    ...
    \answer{
        The 'pad' token will mask to $-\infty$, if compute softmax of this value will yield value 0. It also mean this part ('pad' token) takes no effects on the softmax.

        Because 'pad' token is not even exist in real, so It should cut the effects that It will cause.
    }
...
\end{lstlisting}
    ...
    \answer{
        Advantage : Dot product attention is much faster.

        Disadvantage : Dot product don't have extra learnable parameters, so the capacity may less then multiplicative attention.
    }
    \subpart[2] Explain one advantage and one disadvantage of \textit{additive attention} compared to multiplicative attention.
    \answer{
        Advantage : Additive attention is better for small-scale applications. Performs is better.

        Disadvantage : Additive attention is more computationally expensive due to addition and non-linearity.
    }
    \end{subparts}
