...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ### TODO
    bs, nh, seq_len, hs = key.shape
    att = query @ key.transpose(-2, -1) / np.sqrt(self.attention_head_size)
    att += attention_mask
    att = torch.softmax(att, dim=-1)
    att = self.dropout(att)
    y = att @ value
    y = y.transpose(1, 2).contiguous().view(bs, seq_len, nh*hs)
    return y
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    add_norm_out1 = self.add_norm(hidden_states, att_out, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    ffw_out = self.interm_af(self.interm_dense(add_norm_out1))
    add_norm_out2 = self.add_norm(add_norm_out1, ffw_out, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    return self.embed_dropout(self.embed_layer_norm(out))
