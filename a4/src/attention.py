...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    i_dim = 1/10000 ** (-2 * torch.arange(dim/2, dtype=torch.float32) / dim)
    t_dim = torch.arange(max_positions, dtype=torch.float32)
    rope_cache = torch.outer(t_dim, i_dim)[..., torch.newaxis].repeat(1, 1, 2)
    rope_cache[:, :, 0].cos_()
    rope_cache[:, :, 1].sin_()
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    ### YOUR CODE HERE ###
    B, nh, T, hs = x.size()
    rope_cache_trun = torch.view_as_complex(rope_cache[:T])
    x_complex = torch.view_as_complex(x.view(B, nh, T, hs//2, 2))
    rotated_x = torch.view_as_real(x_complex * rope_cache_trun).view(B, nh, T, hs)
    ### END YOUR CODE ###
    return rotated_x
