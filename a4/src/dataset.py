...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        doc_len = len(doc)
        trun_len = min(random.randint(4, int(self.block_size*7/8)), doc_len)
        doc = doc[: trun_len]
        
        pos1 = random.randint(0, trun_len)
        pos2 = random.randint(pos1, trun_len)
        prefix = doc[: pos1]
        masked_content = doc[pos1: pos2]
        suffix = doc[pos2: ]
        pad_len = self.block_size - trun_len - 1
        assert pad_len >= 0, 'Padding length must greater equal to 0'
