...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{
            \alns{
                ...
                K_{\T{prem}} &= PXW_K = P K\\
                V_{\T{prem}} &= PXW_V = P V\\
                H_{\T{prem}} &= P \T{softmax}(\frac{QK^T}{\sqrt{d}}) P^T P V\\
                \because P^T &= P^{-1} \Rightarrow P^T P = I \\
                H_{\T{prem}} &= P H  \\
                Z_{\T{prem}} &= \relu(P(HW_1 + P^{-1}1\cdot b_1))W_2 + 1 \cdot b_2 \\
                &= P\relu(HW_1 + 1\cdot b_1)W_2 + 1 \cdot b_2 \quad \T{because $P^{-1}$ is still a premution matrix, so $P^{-1} 1 = 1$}\\
                &= P(\relu(HW_1 + 1\cdot b_1)W_2 + P^{-1} 1 \cdot b_2) \\
                &= P Z
            }
        }
        ...
        \ifans{
            Transformer don't take positional information. When you switch two identical word in a sentence, the result is same. But in pratical, the former and after word has different meaning.
        }
    \end{subparts}
    ...
    \begin{subparts}
        ...
        \ifans{
            Yes it will. When add positional embedding to the word vector at different position but identical word, and switch them will make it different.
        }
        ...
        \ifans{
            Pick $a \neq b$,
            \alns{
                \phi_a &= [\sin(a/10000^{2 \times 0 / d}), \cos(a/10000^{2 \times 0 / d}), \dots, \sin(a/10000^{2 \times (d/2 - 1)  / d}), \cos(a/10000^{2 \times  (d/2 - 1) / d})] \\
                \phi_b &= [\sin(b/10000^{2 \times 0 / d}), \cos(b/10000^{2 \times 0 / d}), \dots, \sin(b/10000^{2 \times (d/2 - 1)  / d}), \cos(b/10000^{2 \times  (d/2 - 1) / d})] \\
                \T{let } \phi_{(a, 0)} &= \phi_{(b, 0)} \Rightarrow \sin(a) = \sin(b) \\
                a &= b + 2\pi k \quad \T{or} \quad a = \pi - b + 2\pi k \quad k \in \BZ \\
                a &\in \BZ, b \in \BZ, a \neq b \\
                &\T{The upper equation never holds}
            }
        }
    \end{subparts}
\end{parts}