...
\answer{
    \alns{
        -\sum_{w \in \text{Vocab}} \by_w \log(\byt_w) = - 1 \cdot \log(\byt_o) = \log(\hat{\by}_o)
    }
}
...
\answer{
    $\begin{aligned}[t]
        \nabla_{\bv_c} \bJ_{\T{naive-softmax}} &= \nabla_{\bv_c} -\bu_o^T\bv_c + \log(\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)) \\
        &= -\bu_o + \frac{\nabla_{\bv_c} \sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)}{\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)} \\
        &= -\bu_o + \frac{\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)\bu_w}{\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)} \\
        &= -\bU\by + \bU\byt = \bU(\byt-\by)
    \end{aligned}$
}
...
\answer{
    $\begin{aligned}[t]
        \nabla_{\bv_c} \bJ_{\T{naive-softmax}} &= \sum_{w \in \T{Vocab}} \bu_w (\byt_w - \by_w) \\
        \bu_w (\byt_w - \by_w) &= \begin{cases}
            \bu_w (\frac{\exp(\bu_w^T\bv_c)}{\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)} - 1), & \T{if $w = o$} \\
            \bu_w \frac{\exp(\bu_w^T\bv_c)}{\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)}, & \T{otherwise}
        \end{cases}
    \end{aligned}$ 

    The first term will make $P(W = w | C = c) (\T{When $w = o$})$ close to 1, second term will make $P(O = c | C = c) (\T{When $w \neq o$})$ close to 0.
} 
\end{enumerate}
...
\answer{
    When $\bu_x = \alpha \bu_y, (\alpha > 0)$ then the normalized word vector is same. By the way this means normalization take away useful information (i.e. the magnitude). 

    When $\forall_{x, y}  \bu_x \neq \alpha \bu_y$ then the normalized will not take away the information.
}
...
\answer{
    \alns{
        \nabla_{\bu_w} \bJ_{\T{naive-softmax}}(\bv_c, o, \bU) &= \nabla_{\bu_w} [ -\bu_o^T \bv_c + \log(\sum_{w \in \T{Vocab}} \exp(\bu_w^T\bv_c)) ] \\
        &= \begin{cases}
            \bv_c - \byt_w \bv_c, & \T{if $o = w$} \\
            - \byt_w \bv_c, & \T{otherwise}
        \end{cases}
    }
}
...
\answer{
    \vspace*{1em}
    \alns{
        \nabla_{\bU} \bJ_{\T{naive-softmax}}(\bv_c, o, \bU) &= 
        \begin{bmatrix} -\byt_1\bv_c & \dots & \overmat{$o$th vector}{\bv_c - \byt_w \bv_c} & \dots & -\byt_{|\T{Vocab}|}\bv_c
        \end{bmatrix} \\
        &= \bv_c (\by - \byt)^T
    }
}
