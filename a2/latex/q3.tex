...
\begin{parts}
    ...
    \answer{
        At first each word will \texttt{SHIFT} once. Also will generate $n-1$ normal edge plus one ROOT edge. And a initial configuration step.

        So totally if a sentence containing $n$ words, the step to generate dependency tree will be $2n + 1$.
    }
    ...
    \answer{
        In file \href{run:../parser_transitions.py}{\texttt{parser\_transitions.py}}
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \answer{
            \alnst{
                \pd{\CE(\by, \byt)}{\bl_i} &= \inv{\byt_c}\pd{\byt_c}{\bl_i} \\
                \byt_c &= \frac{\exp(\bl_c)}{\sum_i \exp(\bl_i)} \\
                \pd{\byt_c}{\bl_i} &= \begin{cases}
                    \frac{\exp(\bl_c)}{\sum_j \exp(\bl_j)} - \frac{(\exp(\bl_c))^2}{(\sum_j \exp(\bl_j))^2}, & \T{if $i = c$} \\
                    -\frac{\exp(\bl_i) \exp(\bl_c)}{(\sum_j \exp(\bl_j))^2}, & \T{otherwise}
                \end{cases} \\
                \pd{\byt_c}{\bl_i} &= \begin{cases}
                    \byt_c-\byt_c^2, & \T{if $i = c$} \\
                    -\byt_i \byt_c, & \T{otherwise}
                \end{cases} \\
            }
        }
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \end{enumerate}
    \answer{
        In file \href{run:../parser_model.py}{\texttt{parser\_model.py}} and \href{run:../run.py}{\texttt{run.py}} \\
        The best UAS in dev set is 88.63, in test set is 89.07.
    }
...
\answer{
    Sometimes one word will have different POS depends on the context. If give the model this information, It will increase the accuracy.
}
